...
void Invite::execute() {
  ...
  if (invitee_ptr == NULL) {  // server에 초대하고자 하는 client가 없음.
    ...
    return _client->write(
        ":" + _server->getServerName() + " " +
        Error::err_nosuchnick(_client->getNickname(), _args[0]));
  }
  ...
  if (channel_ptr->findClient(invitee_ptr->getFd()) &&
      _client->findChannel(_args[1])) {  // 443 channel에 이미 invitee가 존재.
    return _client->write(
        ":" + _server->getServerName() + " " +
        Error::err_useronchannel(_client->getNickname(), _args[0], _args[1]));
  }
