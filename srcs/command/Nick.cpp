...
void Nick::execute()
{
	...
	for (size_t j = 0; j < newNick.length(); ++j)
	{
		if (!isalnum(newNick[j]) && newNick[j] != '-' && newNick[j] != '_')
			throw std::runtime_error(Error::err_erroneousnickname(_client->getNickname(), newNick, _command));
	}
	...
	if (std::find(clientNicknames.begin(), clientNicknames.end(), newNick) != clientNicknames.end())
		throw std::runtime_error(Error::err_nicknamealreadyuse(_client->getNickname(), newNick, _command));
