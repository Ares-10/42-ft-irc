...
public:
	...
	static std::string err_needmoreparams(const std::string &client_name,
	...
	static std::string err_nosuchchannel(const std::string &client_name,
										const std::string &channel_name);

	// 405 : JOIN클라이언트가 최대 채널 수에 가입했기 때문에 명령이 실패
	static std::string err_toomanychannels(const std::string &client_name,
											const std::string &channel_name);
	...
	// 명령이 실패 (+k)
	static std::string err_badchannelkey(const std::string &client_name,
										const std::string &channel_name);

	// 473 : JOIN채널이 [초대 전용] 모드로 설정되어 있고 클라이언트가 채널에 초대
	// 되지 않았거나 초대 예외가 설정되지 않았기 때문에 명령이 실패 (+i)
	static std::string err_inviteonlychan(const std::string &client_name,
										const std::string &channel_name);

	// 476 : JOIN에서 유효하지 않은 channel 형식일 때 발생.
	static std::string err_badchanmask(const std::string &client_name,
										const std::string &command);
