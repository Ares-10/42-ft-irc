...
// symbol부분은 '=' 공개 채널, '@' 는 secret channel인데 우린 +s 옵션이 없으므로 그냥 '=' 로 함.
// (353이랑 세트)이고, 만약 사용자가 해당 채널이 들어가 있지 않으면, 안보내지는 듯. (366만)
...
static std::string rpl_endofname(const std::string &nick_name, const std::string &channel_name);
